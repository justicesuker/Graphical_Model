# -*- coding: utf-8 -*-
"""
Created on Thu Oct 28 13:16:58 2021
...
def compute_posterior(prior, likelihood, y):
    ...
    for i in range(M):
        for j in range(N):
             pro[j,i]=likelihood[y[j],i]
        if pro[j,i]==0:
            log_numi[i]=0
        else:
            log_numi[i]=log_prior[i]+sum(np.log(pro[:,i]))           
    for i in range(M):
        log_answer[i]=log_numi[i]-scipy.misc.logsumexp(log_numi)
    #
...
def compute_movie_rating_likelihood(M):
    ...
    for i in range(M):
        for j in range(M):
            if i==j:
                likelihood[j,i]=2
            else: likelihood[j,i]=1/np.abs(j-i)
    for i in range(M):       
        alpha[i]=np.sum(likelihood[:,i])
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    for i in range(num_movies):
        y=movie_data_helper.get_ratings(movie_id_list[i])
        if num_observations!=-1:         
            if len(y)>num_observations:
                y=y[0:num_observations]
...
def compute_entropy(distribution):
    ...
    entropy=0
    for i in range(l):
        if distribution[i]<=10**(-8):
            entropy=entropy
        else:
            entropy=distribution[i]*np.log2(1/distribution[i])+entropy
